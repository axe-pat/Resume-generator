...
For each of the 11 story slots the model picks one of 4–5 labeled variants. On the PM track, FlairX contributes three distinct stories selected from five families: enterprise interview workflows, avatar infrastructure, commercial operations, Ceipal integration, and candidate sourcing. The flagship AI-PM mix is F-ENTERPRISE + F-AVATAR + F-SOURCING; F-CEIPAL is the API/integration/enterprise-ecosystem swap and F-OPS is the commercial-operations swap. Hevo's H3 remains flexible across Regression / Query Engine / H-MONITORING-AI, while Intuit is fixed to Billing + Incident. The PM prompt currently suppresses the summary because the first Experience entry carries a formal AI Product Manager title; application-specific manual builds may restore one short factual identity line when prior engineering titles create a material PM-recognition risk. Current Fluo proof is selected into one `Venture Product: Fluo` row inside Skills & Interests rather than a standalone section. On the non-PM track, Pass 1 can emit an optional **Projects & Consulting** section (Section 3B) when founder-advisory / L'Oréal / USC proof materially improves non-engineering identity.
...
- QC-13: Long-bullet check — two thresholds. In `run_app.py` smart-cost mode, AI trim only runs on the full-quality path (`fit_score >= 7.8`); balanced and lean jobs keep the warning without spending the trim + re-score calls.
...
- **Fact boundary**: unconfirmed 42%/80% outcomes, retention, deal closure, ARR/ACV lift, pricing adoption, and sourcing adoption are blocked from FlairX bullets.
- **Track-specific contracts** (`freeform_runner.py`): PM now renders FlairX/Gojek/Hevo/Intuit at 3/3/3/2 while the non-PM route retains Gojek/Hevo/Intuit/Optum.
- **End-to-end wiring**: updated strategy context, candidate profile, rewrite/scorer schemas, parser/QC paths, local DOCX parsing, and DOCX company metadata. Added `tests/test_flairx_resume_integration.py`.
