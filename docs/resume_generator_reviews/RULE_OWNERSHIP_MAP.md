...
| Document assembly | Rulebook Section 9, `shared/resume_lint.py` | Archetype distribution, opener/phrase/figure repetition, scale coherence, section consistency, density; consumes route-owned bounds rather than reclassifying bullets |
| Critic | `freeform_scorer.txt` | Diagnoses weak dimensions; does not waive any hard gate |
| Release | `shared/resume_lint.py`, parser, DOCX/PDF renderer | One unambiguous final section set, assembled text equals rendered text, observed one-page artifact, non-averaged ready/fail |
